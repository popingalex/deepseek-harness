...
export class JsonlBackendTracker {
  ...
  /**
   * EH fork extension (replay of ba9560c7fb onto the tracker backend): forget
   * every in-process bookkeeping entry for a session the host is durably
   * resetting. The host removes the physical artifact itself and announces a
   * fresh same-id lifecycle afterwards; a concurrently open handle stops
   * being routed but is not force-closed (the host flushes before resetting).
   */
