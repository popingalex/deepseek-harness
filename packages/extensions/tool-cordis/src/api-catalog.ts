...
export const SERVICE_API: readonly ServiceApiEntry[] = [
  ...
  },
  {
    ...
    methods: [
      ...
      {
        signature: 'async reset(_id: SessionId): Promise<void>',
        description: 'EH fork extension (replayed from ba9560c7fb): forget a session\'s in-process persistence state so the host can durably replace the artifact (maintenance reset). The default refuses: backends with per-session artifacts that support in-place replacement override it. Callers are responsible for removing the physical artifact before a replacement session is announced.',
        parameters: [{ name: '_id', description: 'session whose persistence state would be forgotten; unused by the default refusal.' }],
      },
      {
  ...
  },
  {
    ...
    methods: [
      ...
      },
      {
        ...
        description: 'Replace a live session\'s log with a fresh same-id lifecycle in place: detach the current entry (emitting `session/disposed`), then create the replacement (emitting `session/created`). Maintenance flows that rebuild a session\'s content call this after clearing durable state (see `ctx.sessionPersistence.reset`) so the replacement starts from a clean log instead of appending to the old one.',
        parameters: [{ name: 'id', description: 'the live session id to replace; must currently exist.' }, { name: 'options', description: 'seed events and/or creation metadata for the replacement.' }],
