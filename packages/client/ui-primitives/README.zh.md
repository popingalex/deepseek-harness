...
结构化引用 helper 会把 `EH_REF_V1` 与 legacy `EH_LOCATION_V1` wire 形态解析成文本／引用 segment。`ReferenceChip` 与 `ReferenceText` 生成不含存储 id 的显示与 clipboard 标签；位置标签使用可选名称、六位小数经纬度与可选格式化时间。malformed token 保持原始文本。
