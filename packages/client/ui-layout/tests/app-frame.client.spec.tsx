...
import { NARROW_DEBOUNCE_MS } from '../src/client/columns.ts'
...
beforeEach(() => {
  ...
  nextFrame = 1
  // Fake timers drive the narrow-breakpoint debounce (NARROW_DEBOUNCE_MS):
  // crossing tests advance past the stability window explicitly.
  vi.useFakeTimers()
...
describe('AppFrame normal width concessions', () => {
  it('measures the frame, not the window, before choosing the first-open preference', () => {
    ...
    // The measure flips narrowTarget across the breakpoint; flush the debounce.
    act(() => { vi.advanceTimersByTime(NARROW_DEBOUNCE_MS) })
  ...
  it('auto-collapses only below 1024px and preserves the wide sidebar preference', () => {
    ...
    resize(1023)
    // Breakpoint crossings are debounced (EH replay 6335cc792f): the rail
    // flips only after the stability window elapses.
    act(() => { vi.advanceTimersByTime(NARROW_DEBOUNCE_MS) })
