...
import { computeColumns, NARROW_DEBOUNCE_MS, RIGHTBAR_DEFAULT_RATIO, SIDEBAR_AUTO_COLLAPSE, SIDEBAR_DEFAULT } from './columns.ts'
...
export function AppFrame({
  ...
  // Narrow viewports auto-collapse the sidebar; toggleSidebar consults the
  // store's viewportWidth mirror, so the rendered narrow flag must match the
  // store's within one gesture. Crossings are debounced (NARROW_DEBOUNCE_MS):
  // the frame box can sweep across the breakpoint transiently when an external
  // layout push settles (a sidebar plugin's transitioned margin), and flipping
  // the rail on such a sweep reads as the session list expanding-then-collapsing
  // on release. (EH replay 6335cc792f onto the upstream rightbar frame.)
