...
export interface InputActions {
  ...
  submit(): void
  /** Append one structured reference occurrence at the end of the current
   * draft (Emergency Harness reference dock; the composer chip pipeline
   * owns occurrence bookkeeping). Returns false when insertion is refused
   * (busy admission phases). Optional: only the EH-integrated facade
   * exposes it; plain upstream compositions omit it. */
